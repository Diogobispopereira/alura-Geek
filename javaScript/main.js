...
async function listaCard() {
    try {
        const listaApi = await produtos.produtoLista();
         ...
         // Verifique a estrutura de listaApi no console
         console.log("Lista de produtos:", listaApi);
         if (!Array.isArray(listaApi)) {
            throw new Error("A resposta da API não é uma lista válida.");
        }
...
formulario.addEventListener("submit", async (event) => {
    ...
    try {
        ...
        const novoCard = criarCard(novoproduto);
